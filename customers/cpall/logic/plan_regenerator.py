...
def regenerate_production_plan_bytes(plan_run_id: int) -> bytes:
    ...
    # (2026-09-05): ตอนสร้างแผนครั้งแรกซ่อนถูกต้อง แต่ตอนดาวน์โหลดซ้ำ/regenerate (ฟังก์ชันนี้) ไม่เคยมี
...
def regenerate_logistic_plan_bytes(plan_run_id: int, group_name: str) -> bytes:
    ...
    # — ซ่อนแถวไว้เหมือนตอนสร้างแผนครั้งแรก (ดู logistic_plan_export.py) *** เจอบั๊กจริง (2026-09-05):
    ...
    # เขียนทับเซลล์ "ผู้ส่ง : ..." เสมอ (ดูเหตุผลเดียวกับใน logistic_plan_export.py) — ไม่มีเงื่อนไข
    # "มีข้อมูลรถไหมถึงจะเขียน" อีกแล้ว เขียนทับทุกครั้งกันข้อมูลเก่าจาก template ค้างอยู่
